...
- That is a total of **`r round(sum(bank_details$usd), digits = 2)`** USD.
...
ggplot(data = bank_details,
  ...
  labs(title = 'Current totals in bank (converted to USD)',
       subtitle = paste0('Combined total: ', round(sum(bank_details$usd)), ' USD'),
...
The below shows totals, _per year_. All values are USD. Note that values are only included within the period date range (`r params$start_date` through `r params$end_date`). Also note that the following spouses are absorbed into their partner's totals: Coloma (Joe), Xing (Ben):
...
The below shows totals, broken down by the period pre LLC (prior to Feb 25 2019) and the period post LLC (Feb 25 2019 and beyond).. All values are USD. Note that values are only included within the period date range (`r params$start_date` through `r params$end_date`):
